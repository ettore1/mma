...
\emph{menor \'indice} com valor fracion\'ario, gerando dois ramos:
$x_j \le \lfloor x_j^* \rfloor$ (ramo do \emph{piso}, explorado primeiro) e
$x_j \ge \lceil x_j^* \rceil$ (ramo do \emph{teto}). Cada \emph{passo}
...
\end{cases}
\]
$x_2$ \'e cont\'inua; a ramifica\c{c}\~ao incide apenas sobre $x_1, x_3$.
A relaxa\c{c}\~ao na raiz j\'a fornece valores inteiros para as vari\'aveis
exigidas ($x_1=2$, $x_3=0$) com $x_2=2{,}5$ cont\'inua.
A \'arvore se encerra na raiz, sem necessidade de ramifica\c{c}\~ao.
\input{gen/p1_ex4_table.tex}
\noindent\textbf{Solu\c{c}\~ao \'otima:} $z^{*}=11{,}5$ em $(x_1,x_2,x_3)=(2,\,2{,}5,\,0)$.
...
% ============================================================================
Para os problemas de grafos, foi utilizada a biblioteca \texttt{networkx} do Python para
constru\c{c}\~ao e visualiza\c{c}\~ao dos grafos. Os algoritmos foram implementados em Python.

\textbf{M\'inima \'Arvore Geradora.} Dado um grafo n\~ao-dirigido $G=(V,E,w)$,
deseja-se encontrar $T \subseteq E$ que conecta todos os v\'ertices com $\sum_{e \in T} w_e$ m\'inimo.

\textbf{Fluxo M\'aximo (Edmonds-Karp).} Algoritmo de Ford-Fulkerson que utiliza busca em largura
(BFS) no grafo residual para encontrar caminhos aumentantes a cada itera\c{c}\~ao.

\textbf{Observa\c{c}\~ao.} Os pesos e capacidades foram transcritos das figuras do
enunciado e reproduzidos nos grafos a seguir para confer\^encia.
...
\begin{figure}[H]\centering
  ...
  \caption{Fluxo \'otimo (r\'otulos \emph{fluxo/capacidade}), Exerc\'icio~5.}\end{figure}
