...
\pagestyle{fancy}
\fancyhf{}
%\rhead{\small PLI --- CEMMA}
%\lhead{\small Lista de Exerc\'icios}
\cfoot{\thepage}
...
% --------------------------- CAPA ---------------------------
\begin{center}
  {\Large\textbf{Resolu\c{c}\~ao da Lista de Exerc\'icios}}\\[0.3em]
  {\large Programa\c{c}\~ao Linear Inteira}\\[0.6em]
  Especializa\c{c}\~ao em M\'etodos Matem\'aticos Aplicados (CEMMA)\\
  Prof. Franklin Angelo Krukoski\\[0.4em]
  Junho de 2026
\end{center}
\vspace{0.6em}
\hrule
...
\noindent\textbf{Observa\c{c}\~oes metodol\'ogicas.} As relaxa\c{c}\~oes lineares
e os modelos inteiros foram resolvidos com o solver \textbf{CBC} acessado via
biblioteca \textbf{PuLP} (Python). Os grafos e \'arvores foram constru\'idos com a
biblioteca \textbf{NetworkX} e desenhados com \texttt{matplotlib}. Todo o c\'odigo
encontra-se nos \emph{notebooks} e scripts que foram desenvolvidos em Python.
...
\emph{menor \'indice} cujo valor na relaxa\c{c}\~ao seja fracion\'ario, gerando dois
ramos: $x_j \le \lfloor x_j^{*}\rfloor$ (ramo do \emph{piso}, explorado primeiro)
e $x_j \ge \lceil x_j^{*}\rceil$ (ramo do \emph{teto}). Cada \emph{passo}
...
\noindent\textbf{Prim} (a partir do n\'o 1; em cada itera\c{c}\~ao escolhe-se a
aresta de menor peso que liga a \'arvore a um n\'o externo):
\input{gen/p4_ex1_prim.tex}
\noindent Ambos os algoritmos produzem custo total $=\mathbf{74}$.
...
\noindent\textbf{Kruskal:}
\input{gen/p4_ex2_kruskal.tex}
